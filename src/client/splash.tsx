...
export const Splash = () => {
  ...
  return (
    <div>
      { editMatch && <div className="flex relative flex-col justify-center items-center min-h-screen gap-6 bg-green-400 dark:bg-gray-900 px-4">
        <div className="flex flex-col items-center gap-4 w-full max-w-md bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl">
          <div className='flex flex-row gap-2'><div>Team Home</div><input placeholder='Mexiko'></input></div>
          <div className='flex flex-row gap-2'><div>Score Home</div><input placeholder='3'></input></div>
          <div className='flex flex-row gap-2'><div>Team Away</div><input placeholder='South Africa'></input></div>
          <div className='flex flex-row gap-2'><div>Score Away</div><input placeholder='1'></input></div>
          <div className='flex flex-row gap-2'><div>StartDate</div><input type='datetime-local'></input></div>
          <div className='flex flex-row gap-2'>
            <button className="w-full mt-2 bg-red-700 text-white font-semibold py-2 px-4 rounded-xl transition-colors disabled:opacity-50 disabled:bg-gray-400 cursor-pointer disabled:cursor-not-allowed" onClick={() => {setEditMatch(false)}}>Cancel</button>
            <button className="w-full mt-2 bg-red-700 text-white font-semibold py-2 px-4 rounded-xl transition-colors disabled:opacity-50 disabled:bg-gray-400 cursor-pointer disabled:cursor-not-allowed" onClick={() => {setEditMatch(false)}}>Submit</button>
          </div>
        </div>
      </div>}
      { !editMatch && <div className="flex relative flex-col justify-center items-center min-h-screen gap-6 bg-green-400 dark:bg-gray-900 px-4">
        ...
        <div>
          <button onClick={() => {setEditMatch(!editMatch)}}  className="w-full mt-2 bg-red-700 text-white font-semibold py-2 px-4 rounded-xl transition-colors disabled:opacity-50 disabled:bg-gray-400 cursor-pointer disabled:cursor-not-allowed">Edit Match</button>
        </div>
      </div> }
    </div>
  );
